...
namespace AdamAsmaca.Isler
{
    public class AdamAsmacaMetotlari
    {
        ...
        private static void YeniUyeKontrol(Kullanici model)
        {
            ...
            bool kucukKarakterVarmi = model.Sifre.ToList().Count(x => x >= 'a' && x <= 'z') >= 1;
            if (!kucukKarakterVarmi)
                throw new Exception("Sifre içerisinde en az 1 kücük harf olmak durumunda");

            bool buyukKarakterVarmi = model.Sifre.ToList().Count(x => x >= 'A' && x <= 'Z') >= 1;
            if (!buyukKarakterVarmi)
                throw new Exception("Sifre içerisinde en az 1 büyük harf olmak durumunda");
        }
